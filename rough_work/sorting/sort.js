...
console.log(simpleList.sort().reverse());
...
function searchObject(list, query) {
  function searchCallback(item) {
    let name = item.name.toLowerCase().includes(query.toLowerCase());
    description = item.description.toLowerCase().includes(query.toLowerCase());
    return name || description;
  }
  return list.filter(searchCallback);
}
console.log(searchObject(hikes, "al"));

function sortHikesByDistance(hikes)
{
  return hikes.sort((a, b) => {
    return parseFloat(a.distance) - parseFloat(b.distance);
  });
}
console.log(sortHikesByDistance(hikes));
